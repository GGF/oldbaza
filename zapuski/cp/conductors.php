...
elseif (isset($show) || isset($edit)|| isset($add) )
{
	...
	$r = getright($user);
		...
		if ($edit!=0) {
			...
			echo "Заказчик:<SELECT name=customer_id ".($edit!=0?"readonly":"")." id=cusid onchange=\"var plat=$.ajax({url:'http://".$_SERVER['HTTP_HOST']."/zapuski/zd.php',data:'cusid='+$('#cusid').val()+'&selectplates',async:false}).responseText; $('#myplates').html(plat); $('#myplates').removeAttr('disabled');\">";
			...
			echo "</SELECT><br>";
			echo "Плата:<SELECT name=plate_id disabled id=myplates>";
