...
elseif (isset($print))
{
	if ($print=='mp') 
	{
		...
		$date = date("d-m-Y");
		if (!is_dir(cp1251_to_utf8("/home/common/z/Заказчики/".$customer))) {
			mkdir(cp1251_to_utf8("/home/common/z/Заказчики/".$customer));
			chmod(cp1251_to_utf8("/home/common/z/Заказчики/".$customer),0777);
		}
		if (!is_dir(cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname))) {
			mkdir(cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname));
			chmod(cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname),0777);
		}
		if (!is_dir(cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname."/Мастерплаты"))) {
			mkdir (cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname."/Мастерплаты"));
			chmod (cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname."/Мастерплаты"),0777);
		}
		$filename = cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname."/Мастерплаты/МП-".$date."-".$mp_id.".xml");
		$excel .= file_get_contents("mp.xml");
		$excel = str_replace("_number_",sprintf("%08d",$mp_id),$excel);
		$customer = cp1251_to_utf8($customer);
		$excel = str_replace("_customer_",$customer,$excel);
		$excel = str_replace("_date_",$date,$excel);
		$blockname = cp1251_to_utf8($blockname);
	...
	elseif ($print=='sl') 
	{
		...
		{
		$dir = cp1251_to_utf8("/home/common/z/Заказчики/".$customer);
		if (!is_dir($dir)) {
			mkdir ($dir);
			chmod ($dir,0777);
		}
		$dir = cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname);
		if (!is_dir($dir)) {
			mkdir ($dir);
			chmod ($dir,0777);
		}
		$dir = cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname."/запуски");
		if (!is_dir($dir)) {
			mkdir ($dir);
			chmod ($dir,0777);
		}
		$filename = cp1251_to_utf8("/home/common/z/Заказчики/".$customer."/".$blockname."/запуски/СЛ-".$date."-".$lanch_id.".xml");
		}
		if (isset($dpp)) 
		{
			...
			$res = mysql_query($sql);
			if ($rs=mysql_fetch_array($res)) {
				$comment1 = cp1251_to_utf8($rs["comment"]);
			}
			$sql="SELECT comment FROM coments WHERE id='{$comment_id2}'";
			$res = mysql_query($sql);
			if ($rs=mysql_fetch_array($res)) {
				$comment2 = cp1251_to_utf8($rs["comment"]);
			}
			...
			$excel = str_replace("_type_",($layers=='1'?cp1251_to_utf8("ОПП"):cp1251_to_utf8("ДПП")),$excel);
			...
			$excel = str_replace("_pokr_",($immer=='1'?cp1251_to_utf8("Иммерсионное золото"):cp1251_to_utf8("ПОС61")),$excel);
			...
			$excel = str_replace("_aurum_",($aurum=='1'?cp1251_to_utf8("Золочение контактов"):""),$excel);
			$excel = str_replace("_dozap_",(isset($dozap)?cp1251_to_utf8("ДОЗАПУСК"):""),$excel);
		...
		elseif (isset($mpp))
		{
			...
			if ($rs=mysql_fetch_array($res)) {
				//print_r($rs);exit;
				foreach ($rs as $key => $val) {
					if (true/*!is_utf($val)*/) ${$key}=cp1251_to_utf8($val);
				}
			}
			if (isset($dozap)) 
			{
			...
			$excel = str_replace("_priemo_",(strstr($priem,cp1251_to_utf8("ОТК"))?"+":"-"),$excel);
			$excel = str_replace("_priemp_",(strstr($priem,cp1251_to_utf8("ПЗ"))?"+":"-"),$excel);
			$excel = str_replace("_impokr_",($immer=='1'?"+":"-"),$excel);
			$excel = str_replace("_lamel_",($aurum=='1'?$lamel:"-"),$excel);
			$excel = str_replace("_mark_",$mark,$excel);
			$excel = str_replace("_rmark_",($rmark=='1'?"+":"-"),$excel);
			$excel = str_replace("_maskz_",(strstr($mask,cp1251_to_utf8("Ж"))?"+":"-"),$excel);
			$excel = str_replace("_masks_",(strstr($mask,cp1251_to_utf8("К"))?"+":"-"),$excel);
	...
	elseif ($print="tz") 
	{
		...
		$filelink =  cp1251_to_utf8("/home/common/".str_replace("заказчики","Заказчики",str_replace("\\","/",str_replace(":","",$rs[0]))));
		//echo $filelink;
