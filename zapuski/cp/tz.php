...
if (isset($add) ) {
	if (isset($typetz)) 
	{
		...
		$dir=cp1251_to_utf8("/home/common/t/Расчет стоимости плат/ТехЗад/".$customer);
		if (!is_dir($dir)) {
			mkdir($dir);
			chmod($dir,0777);
		}
		...
		do {
			$filename = cp1251_to_utf8("/home/common/t/Расчет стоимости плат/ТехЗад/".$customer."/".str_replace("\'","-",str_replace("\"","-",str_replace("*","-",str_replace("/","-",str_replace("\\","-",$rs["number"])))))." от ".$rs["orderdate"]." ".$pos_in_order." ".($typetz=="mpp"?"МПП":"ДПП").".xls");
